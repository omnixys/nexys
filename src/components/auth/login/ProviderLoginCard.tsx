...
import type { AuthMethod } from "./LogInPage";
...
export default function ProviderLoginCard({ selected, onSelect }: ProviderLoginCardProps) {
  const t = useTypedTranslations("login");
  const [tab, setTab] = useState<"providers" | "auth">("auth");

  const oauthProviders = providers.filter((p) =>
    ["github", "google", "facebook", "twitter", "linkedin", "keycloak"].includes(p.id),
  );
  ...
  return (
    ...
    >
      ...
      <Stack spacing={1}>
        {(tab === "providers" ? oauthProviders : authMethods).map((provider) => {
          ...
          return (
            <Button
              ...
              sx={{
                ...
                boxShadow: isActive ? "0 0 20px rgba(124,77,255,0.35)" : "none",
                "&:hover": {
